...
保持云存储为“仅创建者和管理员可读写（PRIVATE）”，不要为个人主页开放整个存储桶。上传由服务端 SDK 完成；公开页面保存的是稳定的站内 `/api/media/*` 地址，该路由只允许访问当前环境 `personal-portfolio/` 前缀，并在每次请求时生成短期 CloudBase 签名地址。这样不会改变同环境旅行相册资源的读取边界，也不会把服务端凭据暴露给浏览器。
